...
/obj/machinery/computer/holodeck/initialize()
	...
	if(program_cache.len && init_program == null)
		load_program(pick(program_cache),force=1)

/obj/machinery/computer/holodeck/proc/toggle_power(var/toggleOn = 0)
	if(active == toggleOn) return

	if(toggleOn)
		load_program(offline_program, delay = 1)
		active = 1
	else
